...
int main(int argc, char **argv)
{
	...
	if (debug > 1)
		cebu32_imm_default_dump(0, orig_argv, 0, 0); // prologue

	if (test == 0) {
		while (count--) {
			v = rnd32() & mask;
			old = cebu32_imm_lookup(&ceb_root, v);
			if (old) {
				if (cebu32_imm_delete(&ceb_root, old) != old)
					abort();
				...
				key = calloc(1, sizeof(*key));
				key->key = v;
				old = cebu32_imm_insert(&ceb_root, &key->node);
				if (old != &key->node)
					abort();
			}
		}
		...
		while (count--) {
			...
			if (debug > 1) {
				...
				cebu32_imm_default_dump(&ceb_root, len < sizeof(cmd) ? cmd : orig_argv, old, round + 1);
				round++;
			}
		}
	} else if (test == 2) {
		while (count--) {
			v = rnd32() & mask;
			if (!count && debug > 2)
				cebu32_imm_default_dump(&ceb_root, "step1", 0, (count + 1) * 6 + 1);
			old = cebu32_imm_pick(&ceb_root, v);
			if (!count && debug > 2)
				cebu32_imm_default_dump(&ceb_root, "step2", 0, (count + 1) * 6 + 2);
			back = old;
			while (old) {
				if (old && !count && debug > 2)
					cebu32_imm_default_dump(&ceb_root, "step3", 0, (count + 1) * 6 + 3);
				old = cebu32_imm_pick(&ceb_root, v);
				//if (old)
			...
			if (!count && debug > 2)
				cebu32_imm_default_dump(&ceb_root, "step4", 0, (count + 1) * 6 + 4);
			...
			if (!count && debug > 2)
				cebu32_imm_default_dump(&ceb_root, "step5", 0, (count + 1) * 6 + 5);
			else if (debug > 1) {
				static int round;
				char cmd[100];
				size_t len;
				...
				len = snprintf(cmd, sizeof(cmd), "%s %d/%d : %p %d\n", orig_argv, round, round+count, old, v);
				cebu32_imm_default_dump(&ceb_root, len < sizeof(cmd) ? cmd : orig_argv, old, (count + 1) * 6 + 6);
				round++;
			}
		}
	}
	...
	if (debug > 1)
		cebu32_imm_default_dump(0, 0, 0, 0); // epilogue

	if (debug == 1)
		cebu32_imm_default_dump(&ceb_root, orig_argv, 0, 0);
	return 0;
}
