...
int main(int argc, char **argv)
{
	...
	while (loops-- > 0) {
		...
		for (i = 0; i < lookups; i++) {
			...
			old = cebs_imm_lookup(&ceb_root, kptr);
			if (old)
				found++;
		}
	}
	...
	/* now count elements */
	found = 0;
	ret = cebs_imm_first(&ceb_root);
	if (debug > 1)
		fprintf(stderr, "%d: ret=%p\n", __LINE__, ret);

	while (ret) {
		prev = cebs_imm_next(&ceb_root, ret);
		if (debug)
	...
	for (i = 0, old = NULL, node = cebs_imm_first(&ceb_root); node; i++, node = cebs_imm_next(&ceb_root, (struct ceb_node*)(old = node))) {
		if (node == old) {
			printf("# BUG! next(%p) = %p!\n", old, node);
			exit(1);
		}
	...
	for (i = 0, old = NULL, node = cebs_imm_last(&ceb_root); node; i++, node = cebs_imm_prev(&ceb_root, (struct ceb_node*)(old = node))) {
		if (node == old) {
			printf("# BUG! prev(%p) = %p!\n", old, node);
			exit(1);
		}
	...
	if (!debug && dump)
		cebs_imm_default_dump(&ceb_root, orig_argv, 0, 0);

	printf("# Removing all keys one at a time\n");
	for (old = NULL; (node = cebs_imm_first(&ceb_root)); old = node) {
		if (node == old) {
			printf("# BUG! first() after delete(%p) = %p!\n", old, node);
			exit(1);
		}
		cebs_imm_delete(&ceb_root, (struct ceb_node*)node);
	}
