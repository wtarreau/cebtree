...
struct ceb_node *add_value(struct ceb_root **root, uint32_t value)
{
	...
	key->key = value;
	do {
		prev = ceb32_imm_insert(root, &key->node);
		if (prev == &key->node)
		...
		ret = ceb32_imm_delete(root, prev);
		if (ret != prev) {
...
int main(int argc, char **argv)
{
	...
	if (debug)
		ceb32_imm_default_dump(0, orig_argv, 0, 0); // prologue

	while (argc > 0) {
		v = atoi(argv[0]);
		old = ceb32_imm_lookup(&ceb_root, v);
		if (old)
		...
		if (debug) {
			...
			ceb32_imm_default_dump(&ceb_root, len < sizeof(cmd) ? cmd : orig_argv, old, debug);
			debug++;
		}
	...
	for (i = 0, old = NULL, node = ceb32_imm_first(&ceb_root); node; i++, node = ceb32_imm_next(&ceb_root, (struct ceb_node*)(old = node))) {
		if (node == old) {
			printf("# BUG! prev(%p) = %p!\n", old, node);
			exit(1);
		}
	...
	for (i = 0, old = NULL, node = ceb32_imm_last(&ceb_root); node; i++, node = ceb32_imm_prev(&ceb_root, (struct ceb_node*)(old = node))) {
		if (node == old) {
			printf("# BUG! prev(%p) = %p!\n", old, node);
			exit(1);
		}
	...
	for (old = NULL; (node = ceb32_imm_first(&ceb_root)); old = node) {
		if (node == old) {
			printf("# BUG! first() after delete(%p) = %p!\n", old, node);
			exit(1);
		}
		ceb32_imm_delete(&ceb_root, (struct ceb_node*)node);
		if (debug) {
			char cmd[100];
			size_t len;

			len = snprintf(cmd, sizeof(cmd), "delete(%p:%d)", node, container_of(node, struct key, node)->key);
			ceb32_imm_default_dump(&ceb_root, len < sizeof(cmd) ? cmd : orig_argv, node, debug);
			debug++;
		}
	}

	if (debug)
		ceb32_imm_default_dump(0, 0, 0, 0); // epilogue
	else
		ceb32_imm_default_dump(&ceb_root, orig_argv, 0, 0);
